...
def apply_sim3_to_chunk_ply(plydata, s, R, t):
    """Apply Sim3 transform (scale s, rotation R, translation t) to a Gaussian PLY.

    PLY attribute conventions (3DGS format):
      x / y / z:         world-space Gaussian centre
      scale_0/1/2:       log-scale (log σ) for each axis
      rot_0/1/2/3:       unit quaternion in wxyz order

    Transform rules:
      centre:     s * R @ xyz + t          (full Sim3)
      log-scales: log_scale += log(s)      (Gaussian footprint scales with scene)
      rotation:   q_new = q_R * q_chunk    (left-multiply by frame rotation)
    """
    s_val = float(s.item()) if hasattr(s, "item") else float(s)
    R_np = R.cpu().numpy() if hasattr(R, "cpu") else np.asarray(R, dtype=np.float64)
    t_np = t.cpu().numpy() if hasattr(t, "cpu") else np.asarray(t, dtype=np.float64)

    vertex = plydata["vertex"].data.copy()

    # --- positions ---
    xyz = np.stack([vertex["x"], vertex["y"], vertex["z"]], axis=1).astype(np.float64)
    xyz_new = s_val * (xyz @ R_np.T) + t_np
    vertex["x"] = xyz_new[:, 0].astype(np.float32)
    vertex["y"] = xyz_new[:, 1].astype(np.float32)
    vertex["z"] = xyz_new[:, 2].astype(np.float32)

    # --- log-scales ---
    log_s = float(np.log(s_val))
    vertex["scale_0"] = (vertex["scale_0"] + log_s).astype(np.float32)
    vertex["scale_1"] = (vertex["scale_1"] + log_s).astype(np.float32)
    vertex["scale_2"] = (vertex["scale_2"] + log_s).astype(np.float32)

    # --- rotations (wxyz → xyzw for scipy, compose, convert back) ---
    r_delta = ScipyRotation.from_matrix(R_np)
    rot_wxyz = np.stack(
        [vertex["rot_0"], vertex["rot_1"], vertex["rot_2"], vertex["rot_3"]], axis=1
    ).astype(np.float64)
    rot_xyzw = rot_wxyz[:, [1, 2, 3, 0]]
    rots_new = r_delta * ScipyRotation.from_quat(rot_xyzw)
    rot_xyzw_new = rots_new.as_quat()
    rot_wxyz_new = rot_xyzw_new[:, [3, 0, 1, 2]]
    vertex["rot_0"] = rot_wxyz_new[:, 0].astype(np.float32)
    vertex["rot_1"] = rot_wxyz_new[:, 1].astype(np.float32)
    vertex["rot_2"] = rot_wxyz_new[:, 2].astype(np.float32)
    vertex["rot_3"] = rot_wxyz_new[:, 3].astype(np.float32)

    return PlyData([PlyElement.describe(vertex, "vertex")], byte_order="<")
...
class DA3_Streaming:
    def __init__(self, image_dir, save_dir, config):
        ...
        self.export_gs = self.config["Model"].get("export_gs", False)
        self.gs_ply_unaligned_dir = os.path.join(save_dir, "_tmp_gs_chunks") if self.export_gs else None
        if self.export_gs:
            os.makedirs(self.gs_ply_unaligned_dir, exist_ok=True)
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=self.dtype):
                ...
                predictions = self.model.inference(
                    ...
                    infer_gs=self.export_gs,
                )

                # Save per-chunk GS PLY before depth is squeezed.
                # save_gaussian_ply expects ctx_depth with shape (v, h, w, 1),
                # which we can only reconstruct from the (1, v, h, w) pre-squeeze depth.
                if self.export_gs and not is_loop and predictions.gaussians is not None:
                    self._save_chunk_gs_ply(predictions, chunk_idx)
                    predictions.gaussians = None  # free GPU memory; GS is on disk now
    ...
    def _save_chunk_gs_ply(self, predictions, chunk_idx):
        """Save Gaussian splat PLY for one chunk in its own (unaligned) coordinate frame.

        Size reduction strategy (why each piece matters):
          1. Overlap-aware frame masking: the last overlap_e frames of every non-final chunk
             are shared with the START of the next chunk. Setting their ctx_depth to NaN
             makes save_gaussian_ply's per-view depth percentile NaN → (depth ≤ NaN) = False
             → all those pixels masked out.  Eliminates ~33% duplicate Gaussians.
          2. gs_views_interval: sample every Nth frame.  interval=3 → ~3× fewer Gaussians.
          3. prune_by_depth_percent: tighter depth cutoff → fewer far-away Gaussians.
        """
        from depth_anything_3.utils.gsply_helpers import save_gaussian_ply

        save_path = os.path.join(self.gs_ply_unaligned_dir, f"chunk_{chunk_idx}.ply")

        depth_np = predictions.depth
        if depth_np.ndim == 4:
            depth_np = depth_np[0]  # (1, v, h, w) → (v, h, w)

        gs_cfg = self.config["Model"].get("Gaussians_Save", {})
        gs_views_interval    = gs_cfg.get("gs_views_interval",    1)
        prune_depth_pct      = gs_cfg.get("prune_by_depth_percent", 0.9)
        prune_border         = gs_cfg.get("prune_border_gs",       True)

        # ctx_depth: (v, h, w, 1) — must be on same device as gaussians tensors.
        ctx_depth = torch.from_numpy(depth_np).float().unsqueeze(-1).to(self.device)

        # --- Overlap-aware masking ---
        # The last overlap_e frames of every non-final chunk are also the first frames
        # of the next chunk.  NaN depth → per-view quantile = NaN → depth ≤ NaN = False
        # → save_gaussian_ply's depth mask zeros out those views entirely.
        n_chunks = len(self.chunk_indices)
        if chunk_idx < n_chunks - 1:
            ctx_depth[ctx_depth.shape[0] - self.overlap_e :] = float("nan")
        ...
        try:
            save_gaussian_ply(
                ...
                inv_opacity=True,
                prune_by_depth_percent=prune_depth_pct,
                prune_border_gs=prune_border,
            )
            n_gs = PlyData.read(save_path)["vertex"].count
            print(f"  [GS] chunk {chunk_idx}: {n_gs:,} Gaussians → {save_path}")
        except Exception as exc:
            print(f"  [GS][WARN] chunk {chunk_idx} export failed: {exc}")

    def save_gs_ply(self):
        """Align per-chunk GS PLYs via the accumulated Sim3 and write combined_gs.ply."""
        if not self.export_gs:
            return

        print("Merging GS PLY chunks...")
        vertex_arrays = []
        n_chunks = len(self.chunk_indices)

        # Chunk 0 lives in the reference frame — no transform needed.
        chunk_0_path = os.path.join(self.gs_ply_unaligned_dir, "chunk_0.ply")
        if os.path.exists(chunk_0_path):
            vertex_arrays.append(PlyData.read(chunk_0_path)["vertex"].data)
        else:
            print("  [GS][WARN] chunk_0.ply not found.")

        # Chunks 1..N-1: apply the accumulated Sim3 from process_long_sequence.
        for chunk_idx in range(1, n_chunks):
            chunk_path = os.path.join(self.gs_ply_unaligned_dir, f"chunk_{chunk_idx}.ply")
            if not os.path.exists(chunk_path):
                print(f"  [GS][WARN] chunk_{chunk_idx}.ply not found, skipping.")
                continue
            s, R, t = self.sim3_list[chunk_idx - 1]
            ply_aligned = apply_sim3_to_chunk_ply(PlyData.read(chunk_path), s, R, t)
            vertex_arrays.append(ply_aligned["vertex"].data)

        if not vertex_arrays:
            print("  [GS][WARN] No GS chunks found — skipping combined_gs.ply.")
            return

        merged = np.concatenate(vertex_arrays)

        # --- Opacity filter ---
        # DA3 generates one Gaussian per pixel; ~95% have sigmoid(opacity) < 0.05.
        # Near-transparent Gaussians from misaligned chunks accumulate in alpha-compositing
        # and cause the blurry ghosting artifacts typical of naive streaming merges.
        # Filter them out here — the remaining Gaussians are the ones that actually render.
        gs_cfg = self.config["Model"].get("Gaussians_Save", {})
        op_thr = gs_cfg.get("opacity_threshold", 0.0)
        if op_thr > 0:
            op_sigmoid = 1.0 / (1.0 + np.exp(-merged["opacity"].astype(np.float64)))
            keep = op_sigmoid >= op_thr
            n_before = len(merged)
            merged = merged[keep]
            print(f"  Opacity filter >= {op_thr}: {len(merged):,} kept / {n_before:,} total "
                  f"({len(merged)/n_before*100:.1f}%)")

        gs_out_dir = os.path.join(self.output_dir, "gs_ply")
        os.makedirs(gs_out_dir, exist_ok=True)
        out_path = os.path.join(gs_out_dir, "combined_gs.ply")
        PlyData([PlyElement.describe(merged, "vertex")], byte_order="<").write(out_path)
        size_mb = os.path.getsize(out_path) / 1024**2
        print(f"Saved combined GS PLY → {out_path}  ({len(merged):,} Gaussians, {size_mb:.0f} MB)")
    ...
    def close(self):
        ...
        if self.gs_ply_unaligned_dir and os.path.exists(self.gs_ply_unaligned_dir):
            print(f"Deleting the temp files under {self.gs_ply_unaligned_dir}")
            for filename in os.listdir(self.gs_ply_unaligned_dir):
                file_path = os.path.join(self.gs_ply_unaligned_dir, filename)
                if os.path.isfile(file_path):
                    total_space += os.path.getsize(file_path)
                    os.remove(file_path)
